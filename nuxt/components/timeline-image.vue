    ...
    <div class="container">
      <div class="row" :class="{ 'flex-row-reverse': left }">
        ...
        <div :class="textImageFirstColClasses" @click="handleClick" :style="{ cursor: href ? 'pointer' : undefined }">
          <div :class="{ 'fade-in-bottom': !noAnimation }"
        ...
        <div :class="textImageSecondColClasses" data-utility-animation-step="1">
          ...
          <div v-if="subline" class="text-image__img text-image__img--subline mb-5 mt-5" justify-content="center">
            <v-img :img="image" :cloudinary="cloudinary" :imgSrcSets="textImageImgSrcSets" :preset="textImagePreset"
              :alt="alt" :lottie="lottie"/>
          </div>
          ...
          <div v-if="list">
            <template v-for="(element, index) in list" :key="index">
              <div class="font-size-1">
                ...
                <cta v-if="element.ctaText" :text="element.ctaText" :href="element.ctaHref" :link="element.link"
                  :target="element.target" :type="element.type" />
              </div>
            </template>
          </div>
        </div>
