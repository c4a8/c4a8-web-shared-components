...
<style>
/*
 * The layers must stay siblings and must not sit inside another backdrop-filter:
 * such an element opens its own backdrop root and the layers would no longer
 * see through to the content behind them.
 */
.blur-fade {
  position: absolute;
  inset: 0;
...
/* painted after the layers, so the tint sits on top of the blurred content */
.blur-fade__tint {
  position: absolute;
  inset: 0;
}
